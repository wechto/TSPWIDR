...
def train(f='train'):
    ...
    for epoch in range(opt.max_epoch):
        ...
        if epoch % 50 == 0:
            ...
            model.train()
            print(epoch, np.round(np.array(epoch_losses_train[-1]),5), np.round(np.array(pre_out[0]),5),
                  np.round(np.array(pre_out[1]),5), np.round(np.array(pre_out[2]),5), opt.lr)
        if epoch < 5:
            continue
        if (epoch_losses_train[-1] > epoch_losses_train[-2]).sum() == opt.recurrent_n+1 :
            opt.lr = opt.lr * opt.lr_decay
        if epoch % 500 == 0:
            model.save(opt, name = str(epoch)+'_'+str(opt.data_list_index)+'-'+str(opt.recurrent_n ))

    path = model.save(opt, name = str(opt.data_list_index)+'-'+str(opt.recurrent_n ))
...
if __name__ == '__main__':
    ...
    opt.max_epoch = 2000
    opt.data_list_index = 2
    opt.recurrent_n = 6
    path_seq2seq = ['Seq2Seq_NINO_0123193051', 'Seq2Seq_NINO_0123193230', 'Seq2Seq_NINO_0123193337', 'Seq2Seq_NINO_0123193437']
#    path_seq2seq = ['Seq2Seq_NINO_0123193051', 'Seq2Seq_NINO_0123193230', 'Seq2Seq_NINO_0518005537', 'Seq2Seq_NINO_0123193437']
    opt.model_list_path[1] = 'checkpoints/'+ path_seq2seq[opt.data_list_index] +'.pth'
